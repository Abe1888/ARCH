...
import { getDocumentUrl } from './documentService';

/**
 * Advanced Document Search Service
 *
 * Provides comprehensive document search capabilities with multiple filter types,
 * relevance scoring, and full-text search across document metadata and OCR text.
 *
 * Search Features:
 * - Full-text search across title, description, filename, and OCR text
 * - Multi-faceted filtering (categories, tags, file types, dates, size)
 * - Relevance scoring and ranking
 * - Fuzzy matching and keyword expansion
 * - Snippet extraction and highlighting
 * - AI-powered search suggestions
 *
 * Search Algorithm:
 * 1. Applies filters to narrow result set
 * 2. Performs full-text matching across multiple fields
 * 3. Calculates relevance scores based on field weights
 * 4. Extracts and highlights matching snippets
...
/**
 * Performs advanced document search with multiple filter types
 *
 * Combines full-text search with structured filters to find documents.
 * Results are ranked by relevance and include highlighted snippets.
 *
 * Filter Precedence:
 * 1. User authentication (always applied)
 * 2. Full-text search query (title, description, filename, OCR text, tags)
 * 3. Category filters (exact match)
 * 4. Tag filters (contains)
 * 5. File type filters (exact match)
 * 6. Date range filters
 * 7. Size range filters
 * 8. Favorite status filter
 *
 * @param filters - Search filters and criteria
 * @returns Search results with relevance scores and snippets
 *
 * @example
 * const { results, totalCount } = await advancedSearch({
 *   query: 'quarterly report',
 *   categoryIds: ['finance-cat-id'],
...
function calculateRelevance(doc: any, query: string): number {
  ...
  if (doc.ocr_text && doc.ocr_text.toLowerCase().includes(queryLower)) {
    ...
    const ocrMatches = queryWords.filter(word => ocrWords.some((w: string) => w.includes(word))).length;
...
export async function saveSearchToHistory(query: string) {
