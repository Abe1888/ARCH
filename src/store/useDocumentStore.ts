...
import { type Document, type Category, type ViewMode, type FilterOptions, type SortBy, type SortOrder } from '../types';
import { getDocuments, getCategories, toggleFavorite as toggleFavoriteAPI, getSharedWithMeDocuments } from '../services/documentService';
...
interface DocumentStore {
  ...
  refreshSharedDocuments: () => Promise<void>;
...
export const useDocumentStore = create<DocumentStore>()(persist(
  ...
  refreshDocuments: async () => {
    ...
    const hasCache = cachedDocuments.length > 0;
    ...
    try {
      ...
      if (error) {
        ...
        const errorMessage = typeof error === 'string' ? error : 'Failed to fetch documents';
        ...
        set({
          ...
          cachedDocuments: mappedDocuments,
          isLoading: false
        });
      }
    } catch (err) {
  ...
  refreshSharedDocuments: async () => {
    try {
      const { data, error } = await getSharedWithMeDocuments();
      if (error) {
        ...
        set({ sharedDocuments: mappedDocuments });
      }
    } catch (err) {
  ...
  getFilteredDocuments: () => {
    ...
    } else if (filters.categoryId === 'recent') {
      const sevenDaysAgo = new Date();
      sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
      filtered = filtered.filter((doc) => doc.uploadedAt >= sevenDaysAgo);
    } else if (filters.categoryId === 'shared') {
      const { sharedDocuments } = get();
      filtered = sharedDocuments;
  ...
  getSharedCount: () => {
    return get().sharedDocuments.length;
  },
  ...
  initializeRealtime: () => {
    realtimeManager.subscribeToDocuments((payload) => {
      ...
      const { documents, cachedDocuments } = get();
      ...
      if (eventType === 'INSERT' && newRecord) {
        ...
        set({
          documents: [...documents, mappedDoc],
          cachedDocuments: [...cachedDocuments, mappedDoc]
        });
        ...
        set({
          ...
          cachedDocuments: cachedDocuments.map((doc) =>
            doc.id === newRecord.id ? updatedDoc : doc
          ),
        });
      }
    ...
    realtimeManager.subscribeToCategories((payload) => {
      const { eventType, new: newRecord } = payload;
      const { categories, cachedCategories } = get();
      ...
      if (eventType === 'INSERT' && newRecord) {
        ...
        set({
          categories: [...categories, mappedCategory],
          cachedCategories: [...cachedCategories, mappedCategory]
        });
      } else if (eventType === 'UPDATE' && newRecord) {
        ...
        set({
          ...
          cachedCategories: cachedCategories.map((cat) =>
            cat.id === newRecord.id ? updatedCategory : cat
          ),
        });
      }
