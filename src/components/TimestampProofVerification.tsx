...
export default function TimestampProofVerification({ documentId, documentHash }: TimestampProofVerificationProps) {
  ...
  async function handleVerify() {
    setVerifying(true);
    try {
      const { data, error } = await verifyDocumentWithBatch(documentId, documentHash);
      if (error) {
  ...
  if (!proof) {
    return (
      <div className="bg-yellow-50 rounded-lg p-6 border border-yellow-200">
        ...
        <p className="text-sm text-yellow-700">
          This document does not have a timestamp proof. Proofs are automatically created on upload.
        </p>
      </div>
    );
  }
